...
function validateObject(test, schema, spec, config, trace) {
    ...
    var concreteClass = poly ? findPolymorphicConcreteClass(schema, trace, test, config, spec) : allOf_1.extendAllAllOfs(schema, config, spec);
...
function findDerivedObjects(abstractClass, config, spec) {
    ...
    return Promise.resolve(Object.getOwnPropertyNames(spec.definitions))
        ...
        return allOf_1.extendAllAllOfs(namedSchema.schema, config, spec)
            .then(function (schema) {
            return {
        ...
        return Promise.resolve(namedSchema.schema.allOf)
            .map(function (ref) { return loader_1.loadSchema(ref, spec, config); })
...
function findPolymorphicConcreteClass(abstractClass, trace, test, config, spec) {
    ...
    return derivedObjects
        ...
        if (derivedObjects.length === 0) {
            return Promise.reject("Confusing discriminator - Schema uses discriminator but there are no other Schemas extending it via allOf and using this discriminator property. Trace: " + result_1.getTraceString(trace));
        }
        return derivedObjects;
    })
